...
export class OverheadProvider {
  async getOverheadInfo(_hookData?: ClaudeHookData): Promise<OverheadInfo | null> {
    try {
      const baselinePath = path.join(os.homedir(), ".claude", "state", "actual-baseline.json");
      const bootPath = path.join(os.homedir(), ".claude", "state", "boot-context.json");
      ...
      const total = typeof baselineData.boot_total === "number"
      ...
      const baseline = typeof baselineData.actual_baseline_est === "number"
        ? baselineData.actual_baseline_est
        : 0;

      const gap = typeof baselineData.gap_tokens === "number"
        ? baselineData.gap_tokens
        : 0;

      const gapPct = typeof baselineData.gap_pct_of_baseline === "number"
        ? baselineData.gap_pct_of_baseline
        : 0;

      const pctOfWindow = typeof baselineData.pct_of_200k_window === "number"
      ...
      return {
        ...
        skillCount: typeof baselineData.skill_count === "number" ? baselineData.skill_count : 0,
        agentCount: typeof baselineData.agent_count === "number" ? baselineData.agent_count : 0,
        dynamicLoad: typeof baselineData.boot_dynamic_load === "number"
